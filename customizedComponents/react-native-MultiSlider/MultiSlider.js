...
import {Animated, StyleSheet, PanResponder, View, Easing} from "react-native";
...
class MultiSlider extends Component {
  ...
  render() {
    ...
    let {leftValue, rightValue, containerSize, trackSize, thumbSize} = this.state;

    let thumbSizes = thumbSize.width * (this.props.allowSameValues
      ? 2
      : 1)
    ...
    let leftThumbLeft = leftValue.interpolate({
      ...
      outputRange: [
        0, containerSize.width - thumbSizes
      ]
    });
    ...
    let rightThumbLeft = rightValue.interpolate({
      ...
      outputRange: [
        containerSize.width - thumbSizes,
        0
      ]
    });
    ...
    return (
      <View
        ...
        <Animated.View
          style={[
          styles.thumb, {
            right: rightThumbLeft,
            backgroundColor: rightThumbColor
          },
          this.props.thumbStyle
        ]}
          onLayout=
          {(x) => this._measureThumb(x)}
  ...
  _handlePanResponderMove(evt, gestureState) {
    ...
    let compareFunc = (lhs, rhs, isGE) => {
      return isGE
        ? (lhs >= rhs)
        : (lhs > rhs)
    }
    let compareTo = this.props.allowSameValues
      ? 0
      : this.props.minSpace
  ...
  _handleMeasure(name, x) {
    ...
    if (this._containerSize && this._trackSize && this._thumbSize) {
      this.setState({containerSize: this._containerSize, trackSize: this._trackSize, thumbSize: this._thumbSize})
    }
  }
  /*--------Measure funcs end-----------*/
  ...
  _getThumbLeft(value : number) {
    ...
    return ratio * (this.state.containerSize.width - (this.state.thumbSize.width * (this.props.allowSameValues
      ? 2
      : 1)));
  }

  _getValue(value) {
    let length = this.state.containerSize.width - (this.state.thumbSize.width * (this.props.allowSameValues
      ? 2
      : 1));
    ...
    if (this.props.step) {
      return Math.max(this.props.minValue, Math.min(this.props.maxValue, this.props.minValue + Math.round(ratio * (this.props.maxValue - this.props.minValue) / this.props.step) * this.props.step));
    } else {
      return Math.max(this.props.minValue, Math.min(this.props.maxValue, ratio * (this.props.maxValue - this.props.minValue) + this.props.minValue));
    }
  }
